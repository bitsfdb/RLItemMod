...
async function init() {
  ...
  try {
    ...
    const config = await invoke("get_config").catch((e) => {
    ...
    invoke("cleanup_temp_files").catch((e) =>
...
async function refreshBackups() {
  ...
  backupContainer.innerHTML =
    '<div style="padding: 40px; text-align: center; color: var(--text-secondary);">Scanning for backups...</div>';
  try {
    const backups = await invoke("get_backups");
    if (backups.length === 0) {
      backupContainer.innerHTML =
        '<div style="padding: 60px; text-align: center; color: var(--text-secondary);">No active modifications detected. Your files are clean.</div>';
      return;
    }
    backupContainer.innerHTML = "";
    backups.forEach((file) => {
      ...
      div.innerHTML = `
                <div>
                    ...
                    <div class="backup-date">Modified Item</div>
                </div>
    ...
    console.error(err);
    backupContainer.innerHTML =
      '<div style="padding: 40px; text-align: center; color: var(--danger);">Failed to retrieve backup list.</div>';
  }
}
  ...
  input.addEventListener("input", (e) => {
    ...
    const matches = items
      .filter((item) => {
        ...
        const matchesTerm =
          term.length < 2 || pName.includes(term) || pAsset.includes(term);
        const matchesCat =
          lockCategory === "All" ||
          pSlot.toLowerCase() === lockCategory.toLowerCase();
...
function validate() {
  ...
  const isUnsupported =
    (ownedItem && (oSlot === "Body" || oSlot === "Goal Explosion")) ||
...
async function handleApply() {
  try {
    ...
    const ownedId = (
      ownedItem.ID !== undefined ? ownedItem.ID : ownedItem.id
    ).toString();
    const wantedId = (
      wantedItem.ID !== undefined ? wantedItem.ID : wantedItem.id
    ).toString();
...
async function handleSaveSettings() {
  ...
  try {
    ...
    showToast(
      "Failed! please report this to the maintainer bitsfdb on the discord support server",
      "error",
    );
    console.error(err);
  }
